...
Each of these Modules can be further divided into individual models that will work on their own if you wish to access one summary aspect etc. 
...
data <- read.csv("EmpAttrVerify.csv")
...
model <- glm(Attrition ~.,family=binomial(link='logit'),data=selected_variables)
...
# Accuracy assessments for the model.
fitted.results <- predict(model,type='response')
fitted.results <- ifelse(fitted.results > 0.5,1,0)
misClasificError <- mean(fitted.results != selected_variables$Attrition)
print(paste('Accuracy',1-misClasificError))

# Creating ROC curve and Displaying the AUC value
library(ROCR)
p <- predict(model, type="response")
pr <- prediction(p, selected_variables$Attrition)
